...
const envVarsSchema = Joi.object()
  .keys({
    ...
    CALLBACK_URL: Joi.string().uri().required().description('Google cb url'),
    JWT_SECRET: Joi.string().required().description('JWT secret key'),
    JWT_ACCESS_EXPIRATION_MINUTES: Joi.number().default(10).description('minutes after which access tokens expire'),
    JWT_REFRESH_EXPIRATION_DAYS: Joi.number().default(10).description('days after which refresh tokens expire'),
